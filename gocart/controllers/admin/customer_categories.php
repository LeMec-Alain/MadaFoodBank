...
//New file:Alain Customer Categories.
class Customer_categories extends Admin_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('Customer_category_model', 'Location_model'));
	...
	function index()
	{
		...
		$data['page_title']	= lang('customer_categories');
		$data['categories']	= $this->Customer_category_model->get_categories_tierd();

		$this->load->view($this->config->item('admin_folder').'/customer_categories', $data);
	}
	...
	function organize($id = false)
	{
		...
		if (!$id)
		{
			...
			redirect($this->config->item('admin_folder').'/customer_categories');
		}
	...
	function form($id = false)
	{
		...
		$data['categories']		= $this->Customer_category_model->get_categories();
		...
		if ($id)
		{
			$category		= $this->Customer_category_model->get_category($id);
		...
		{
			$this->load->view($this->config->item('admin_folder').'/customer_category_form', $data);
		}
		else
		{
...
*/
			...
			$category_id	= $this->Customer_category_model->save($save);
