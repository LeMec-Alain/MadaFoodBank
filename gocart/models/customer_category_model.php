...
Class Customer_category_model extends CI_Model
{
	...
	function get_categories($parent = false)
	{
		...
		$this->db->select('id');
		$this->db->order_by('customer_categories.sequence', 'ASC');

		//this will alphabetize them if there is no sequence
		$this->db->order_by('name', 'ASC');
		$result	= $this->db->get('customer_categories');
	...
	{
		return $this->db->get_where('customer_categories', array('id'=>$id))->row();
	}
	...
	function save($category)
	{
		if ($category['id'])
		{
			...
			$this->db->update('customer_categories', $category);

			return $category['id'];
		}
		else
		{
			$this->db->insert('customer_categories', $category);
	...
	{
		$this->db->where('id', $id);
		$this->db->delete('customer_categories');
